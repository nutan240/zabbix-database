(function(){'use strict';// Compressed representation of the Grapheme_Cluster_Break=Extend
// information from
...
class EditorView {
    ...
    static theme(spec, options) {
        ...
        let result = [theme$1.of(prefix), styleModule.of(buildTheme(`.${baseThemeID}.${prefix}`, spec))];
...
].concat(standardKeymap);const defaults$1 = {
    brackets: ["(", "[", "{", "'", '"'],
    before: ")]}'\":;>"
};
...
const completionKeymapExt = Prec.override(keymap.computeN([completionConfig], state => state.facet(completionConfig).defaultKeymap ? [completionKeymap] : []));/// Comment or uncomment the current selection. Will use line comments
...
const defaultHighlightStyle = HighlightStyle.define({ tag: tags.link,
    ...
    color: "#f00" });class SelectedDiagnostic {
...
function tokensFor(d) {
    return new ExternalTokenizer((input, token) => {
        ...
        if (inString(next, Space)) {
            ...
            token.accept(String$1, readLiteral(input, pos + 1, 39 /* SingleQuote */, true));
        }
        else if ((next == 110 /* n */ || next == 78 /* N */) && next2 == 39 /* SingleQuote */ &&
            d.charSetCasts) {
            token.accept(String$1, readLiteral(input, pos + 1, 39 /* SingleQuote */, d.backslashEscapes));
        }
        else if (next == 95 /* Underscore */ && d.charSetCasts) {
            for (;;) {
                let next = input.get(pos++);
                if (next == 39 /* SingleQuote */ && pos > token.start + 2) {
                    token.accept(String$1, readLiteral(input, pos, 39 /* SingleQuote */, d.backslashEscapes));
                    break;
                }
                if (!isAlpha(next))
                    break;
